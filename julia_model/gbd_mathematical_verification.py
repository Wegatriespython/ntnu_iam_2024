...
class GBDMathematicalVerifier:
    ...
    def setup_symbols(self):
        ...
        self.energy_carriers = ['coal', 'gas', 'oil', 'biomass', 'nuclear', 'hydro', 'wind', 'solar', 'electricity', 'nonelectric']
        ...
        self.theta = symbols('theta', positive=True)  # Energy cost approximation (trillion USD)
    ...
    def extract_energy_subproblem_formulation(self):
        ...
        for s in self.sectors:
            for y in self.years:
                ...
                constraint = Eq(energy_supply + self.demand_slack[(s, y)], 
                              self.S[(s, y)])  # S_bar fixed in subproblem
                constraints.append(constraint)
        
        # Cost accounting constraint
        # sum(cost_annual[y] * discount_factor[y]) = TOTAL_COST
        total_cost_constraint = Eq(
            sum(self.COST_ANNUAL[y] * (1 - self.drate)**(self.duration_period * (i)) 
        ...
        self.energy_subproblem = {
            ...
            'variables': list(self.demand_slack.values()) + [self.TOTAL_COST] + list(self.COST_ANNUAL.values())
        }
    ...
    def extract_master_problem_formulation(self):
        ...
        for y in self.years:
            if y not in [2020, 2080]:  # Regular periods
                utility_terms.append(self.udf[y] * sp.log(self.C[y]) * self.duration_period)
        
        # Terminal period correction
        if 2080 in self.years:
            finite_time_corr = symbols('finite_time_corr_2080', positive=True)
            utility_terms.append(self.udf[2080] * sp.log(self.C[2080]) * 
        ...
        for y in self.years:
            if y != 2020:
                ces_term = (self.a * self.KN[y]**(self.rho * self.kpvs) * self.newlab[y]**(self.rho * (1 - self.kpvs)) +
                           self.b * self.NEWENE[('ELEC', y)]**(self.rho * self.elvs) * 
                           self.NEWENE[('NELE', y)]**(self.rho * (1 - self.elvs)))**(1/self.rho)
        ...
        for i, y in enumerate(self.years):
            if y != 2020:
                ...
                constraints.append(Eq(self.K[y], 
                                    self.K[y_prev] * (1 - self.depr)**self.duration_period + self.KN[y]))
        
        # Production accumulation: Y[y] = Y[y-1] * (1-depr)^T + YN[y]
        for i, y in enumerate(self.years):
            if y != 2020:
                y_prev = self.years[i-1]
                constraints.append(Eq(self.Y[y], 
                                    self.Y[y_prev] * (1 - self.depr)**self.duration_period + self.YN[y]))
        ...
        for s in self.sectors:
            for i, y in enumerate(self.years):
                if y != 2020:
                    y_prev = self.years[i-1]
                    constraints.append(Eq(self.PRODENE[(s, y)], 
                                        self.PRODENE[(s, y_prev)] * (1 - self.depr)**self.duration_period + 
                                        self.NEWENE[(s, y)]))
        ...
        for s in self.sectors:
            for y in self.years:
                if y != 2020:
                    constraints.append(Eq(self.S[(s, y)], 
        ...
        for y in self.years:
            if y != 2020:
                disc_y = (1 - self.drate)**(self.duration_period * (self.years.index(y)))
                constraints.append(Eq(self.EC[y], 
                                    self.theta * (self.cost_MESSAGE[y] / totW) / (disc_y * self.duration_period)))
        ...
        self.master_problem = {
            'objective': objective,
            'constraints': constraints,
            'variables': (list(self.S.values()) + list(self.K.values()) + list(self.C.values()) + 
                         list(self.I.values()) + [self.theta])
        }
    ...
    def generate_verification_report(self):
        ...
        with open('/home/weega/ntnu_iam_2024/julia_model/gbd_verification_report.json', 'w') as f:
            json.dump(report, f, indent=2, default=str)
        
        # Generate LaTeX summary
        latex_summary = self.generate_latex_summary()
        with open('/home/weega/ntnu_iam_2024/julia_model/gbd_verification_summary.tex', 'w') as f:
            f.write(latex_summary)
        
        return report
    
    def generate_latex_summary(self):
        """Generate LaTeX summary of mathematical formulation"""
        latex_content = r"""
...
\end{document}
    ...
    def run_verification(self):
        ...
        print(f"LaTeX summary saved to: gbd_verification_summary.tex")
...
if __name__ == "__main__":
    ...
    print("\nNext Steps:")
    print("1. Run debugging infrastructure to test identified issues")
    print("2. Implement component-wise testing")
    print("3. Compare with integrated formulation")
    print("4. Apply fixes based on findings")