...
@with_kw struct CESProduction
    ρ::Float64 = -0.233      # CES exponent (esub = 0.3)
...
function load_macro_parameters(;config_file::String = "")
    # Load from macro_data_load.jl equivalent
    ...
    for i in 2:length(years)
        y_curr = years[i]
        y_prev = years[i-1]
        growth_factor[y_curr] = growth_factor[y_prev] * (1 + grow[y_curr])^10.0
    end
    
    # Labor supply
    labor = Dict{Int,Float64}()
    for (y, gf) in growth_factor
        labor[y] = gf  # Labor grows with economy
    end
    ...
    for s in ["ELEC", "NELE"]
        ...
        for i in 2:length(years)
            y_curr = years[i]
            y_prev = years[i-1]
            aeei_factor[(s, y_curr)] = ((1 - aeei[(s, y_curr)])^10.0) * aeei_factor[(s, y_prev)]
        end
    end
